"""G3 — Heatmap Δ_z: tramo_antiguedad × tipo de formación"""
import nbformat as nbf

nb  = nbf.v4.new_notebook()
CSV = r"c:\Users\r.gonzalez_fluxsolar.LAPTOP-FLUX-ECO\Downloads\Analisis_UCEN_v2\PROCESADO\p3_sat_zscore.csv"
OUT = r"c:\Users\r.gonzalez_fluxsolar.LAPTOP-FLUX-ECO\Downloads\Analisis_UCEN_v2\NOTEBOOKS\G3_heatmap_antiguedad.png"

nb.cells = [

nbf.v4.new_markdown_cell("""\
# G3 — Heatmap Δ Z-score: Antigüedad × Tipo de formación
¿Los docentes más nuevos responden distinto a cada tipo de capacitación?
...
ORD_ANT  = ["0-4","5-9","10-14","15-19","20-24","30+"]
ORD_TIPO = ["TALLER","DIPLOMADO","PROYECTO"]

piv_d = (df.groupby(["tramo_antiguedad","tipo_principal"])["delta_z"]
           .mean().unstack().reindex(index=ORD_ANT, columns=ORD_TIPO))
piv_n = (df.groupby(["tramo_antiguedad","tipo_principal"])["delta_z"]
           .count().unstack().reindex(index=ORD_ANT, columns=ORD_TIPO).fillna(0))

print("Delta Z por antigüedad × tipo:")
...
rows = [r for r in ORD_ANT  if r in piv_d.index]
...
ax.set_ylabel("Tramo de antigüedad (años)", fontsize=15)
ax.set_title("Δ Z-score SAT\\nAntigüedad × Tipo de formación", pad=14, fontsize=19, fontweight="bold")
...
out = r"c:\Users\r.gonzalez_fluxsolar.LAPTOP-FLUX-ECO\Downloads\Analisis_UCEN_v2\NOTEBOOKS\G3_heatmap_antiguedad.ipynb"
