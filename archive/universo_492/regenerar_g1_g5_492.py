...
CSV  = os.path.join(BASE, "..", "PROCESADO", "p3_sat_zscore.csv")
...
print(f"CSV cargado: {len(df)} docentes | universo 492")
print(f"Tipo principal:\n{df['tipo_principal'].value_counts().to_string()}\n")

# ─────────────────────────────────────────────────────────────────────────────
# G1 — Trayectoria z_pre → z_durante → z_post
# ─────────────────────────────────────────────────────────────────────────────
...
MOMENTOS = {"z_pre": "Pre-formación", "z_durante": "Durante", "z_post": "Post-formación"}
...
ax.axhline(0, color="gray", linewidth=1, linestyle="--", alpha=0.6, label="Promedio facultad (z=0)")
ax.set_title("Trayectoria Z-score SAT\npre → durante → post capacitación — Universo 492",
...
plt.tight_layout()
out1 = os.path.join(BASE, "G1_linea_z.png")
plt.savefig(out1, dpi=150, bbox_inches="tight")
plt.close()
print(f"Guardado: {out1}")

# ─────────────────────────────────────────────────────────────────────────────
# G2 — Barras Δz y % mejora por tipo
# ─────────────────────────────────────────────────────────────────────────────
...
fig.suptitle("Impacto de la formación por tipo — Universo 492\n(Z-score SAT con CM-1 y CM-2)",
...
plt.tight_layout()
out2 = os.path.join(BASE, "G2_barras_tipo.png")
plt.savefig(out2, dpi=150, bbox_inches="tight")
plt.close()
print(f"Guardado: {out2}")

# ─────────────────────────────────────────────────────────────────────────────
# G3 — Heatmap antigüedad × tipo
# ─────────────────────────────────────────────────────────────────────────────
ORD_ANT  = ["0-4","5-9","10-14","15-19","20-24","25-29","30+"]
ORD_TIPO = ["TALLER","DIPLOMADO","PROYECTO"]

piv_d3 = (df.groupby(["tramo_antiguedad","tipo_principal"])["delta_z"]
           .mean().unstack().reindex(index=ORD_ANT, columns=ORD_TIPO))
piv_n3 = (df.groupby(["tramo_antiguedad","tipo_principal"])["delta_z"]
...
ax.set_ylabel("Tramo antigüedad (años)", fontsize=14)
ax.set_title("Δ Z-score SAT — Antigüedad × Tipo\nUniverso 492 (CM-1 y CM-2)",
...
fig.text(0.5, -0.02, "Negrita = n ≥ 10  ·  * = n < 5  ·  — = sin datos",
         ha="center", fontsize=11, color="gray")
plt.tight_layout()
out3 = os.path.join(BASE, "G3_heatmap_antiguedad.png")
plt.savefig(out3, dpi=150, bbox_inches="tight")
plt.close()
print(f"Guardado: {out3}")

# ─────────────────────────────────────────────────────────────────────────────
# G4 — Heatmap jerarquía × tipo
# ─────────────────────────────────────────────────────────────────────────────
jer_map = {
    "INSTRUCTOR DOCENTE": "Instructor Doc.", "ASISTENTE DOCENTE": "Asist. Docente",
    "ASISTENTE REGULAR":  "Asist. Regular",  "ASOCIADO DOCENTE":  "Asoc. Docente",
    "ASOCIADO REGULAR":   "Asoc. Regular",   "TITULAR DOCENTE":   "Titular Docente",
...
ax.set_title("Δ Z-score SAT — Jerarquía × Tipo\nUniverso 492 (CM-1 y CM-2)",
             pad=12, fontsize=16, fontweight="bold")
...
fig.text(0.5, -0.02, "Negrita = n ≥ 10  ·  * = n < 5  ·  — = sin datos",
         ha="center", fontsize=11, color="gray")
plt.tight_layout()
out4 = os.path.join(BASE, "G4_heatmap_jerarquia.png")
plt.savefig(out4, dpi=150, bbox_inches="tight")
plt.close()
print(f"Guardado: {out4}")

# ─────────────────────────────────────────────────────────────────────────────
# G5 — Barras sexo × tipo  (todos los que tienen sexo)
# ─────────────────────────────────────────────────────────────────────────────
df5 = df[df["sexo_norm"].isin(["F","M"])].copy()
...
SEX_COLOR = {"F":"#E91E63","M":"#2196F3"}
x     = np.arange(len(TIPOS))
width = 0.35

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
fig.suptitle("Δ Z-score SAT por Sexo × Tipo de formación — Universo 492\n(CM-1 y CM-2 aplicados)",
...
for i, sexo in enumerate(SEXOS):
    ...
    bars1 = ax1.bar(x + offset, np.nan_to_num(vals_dz),  width, label=SEX_LABEL[sexo], color=color, alpha=0.85)
    bars2 = ax2.bar(x + offset, np.nan_to_num(vals_pct), width, label=SEX_LABEL[sexo], color=color, alpha=0.85)
...
plt.tight_layout()
out5 = os.path.join(BASE, "G5_barras_sexo.png")
plt.savefig(out5, dpi=150, bbox_inches="tight")
plt.close()
print(f"Guardado: {out5}")

print("\n" + "="*55)
print("PASO 7 COMPLETO — G1-G5 regenerados con CM-1 y CM-2")
print("="*55)
