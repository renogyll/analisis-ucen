...
CAMBIOS V3.1:
  ...
  - ELIMINAR slide_17 (Cobertura tipo, redundante con Venn)
  - slide_18→17, 19→18, 20→19, 21→20, 22→21, 23→22, 25→23, 26→24
  - slide_18 (nuevo): n DENTRO de cada barra
  - slide_19 (nuevo): "Combinado"→"Participación Mixta"
  - slide_21 (nuevo): encoding fix via _fac() global
  - slide_22 (nuevo): anotaciones compactas DENTRO del gráfico
  - ELIMINAR slide_24 (SAT por Facultad)
  - slide_23 (nuevo): Trayectoria Pura con delta visible (flecha + sombreado)
  - slide_24 (nuevo): Delta z como gráfico divergente % mejora vs % empeora
...
OUT_PPTX  = (r"c:\Users\r.gonzalez_fluxsolar.LAPTOP-FLUX-ECO"
             r"\Downloads\Analisis_UCEN_v2\PRESENTACION_197_P3_v3_slides1_26.pptx")
...
if has_edd:
    ...
    edd_ctrl = (edd_df[edd_df["rut_key"].isin(ruts_917) & ~edd_df["rut_key"].isin(ruts_form_set)
...
def slide_14(prs):
    """Embudo 917→357→197 — ejes reducidos para que bullets no se superpongan."""
    fig = _tr_fig()
    # Embudo más pequeño: usa PIC_RECT con margen horizontal amplio y margen vertical
    ex = PIC_RECT[0] + PIC_RECT[2]*0.12
    ey = PIC_RECT[1] + PIC_RECT[3]*0.06
    ew = PIC_RECT[2] * 0.76
    eh = PIC_RECT[3] * 0.84
    ax  = fig.add_axes([ex, ey, ew, eh], facecolor="none", zorder=5)
    ax.set_xlim(0,10); ax.set_ylim(0,10); ax.axis("off")
    steps = [(9.0,2.8,"917","Docentes jerarquizados UCEN","#5C9BD6"),
             (7.5,2.3,"357","Participaron en ≥1 formación (39%)","#64B5F6"),
             (4.0,1.3,"197","Aptos P3: SAT baseline + resultado  (55% de 357)","#A5D6A7")]
    tops  = [9.8, 7.5, 5.5]
    for i,((tw,bw,n,label,col),top) in enumerate(zip(steps,tops)):
        ...
        ax.text(5+tw/2+0.2,top-0.55,label, ha="left",va="center",fontsize=9.5,
                color="white",zorder=3)

    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"14_embudo.png"), prs)
    _T(sl, "Embudo de Selección — De 917 a 197 Aptos P3")
    _POP(sl, "Criterio Aptos P3: SAT válido en baseline (t-1) y resultado (t+1) de la formación")
    _BUL(sl, [
        "De los 917 docentes jerarquizados, 357 (39%) participaron en alguna iniciativa "
        "de formación entre 2022 y 2025.",
        "197 de esos 357 tienen SAT disponible en los dos momentos clave (baseline y resultado). "
        "Son el universo rector de todo el análisis P3.",
        "Todos los 197 son formados. El grupo de comparación (control externo) proviene de "
        "docentes sin formación con SAT disponible (n=486 docentes, ~300–415 por período).",
    ])
    print("  ✓ slide 14 — Embudo")
...
def slide_16(prs):
    """Evolución acumulativa — centrado (via CHART_X global corregido)."""
    ruts_197 = set(sat["rut_key"].astype(str))
    ev197 = p3ev[p3ev["rut_key"].isin(ruts_197)].copy()
    ev197["anio_evento"] = pd.to_numeric(ev197["anio_evento"], errors="coerce")
    first = (ev197.groupby("rut_key")["anio_evento"].min()
             .reset_index().rename(columns={"anio_evento":"anio_first"}))
    yc = first.groupby("anio_first")["rut_key"].count().sort_index()
    years  = yc.index.astype(int).tolist()
    nuevo  = yc.values.tolist()
    cumul  = np.cumsum(nuevo).tolist()

    fig,ax = _ax()
    xa = np.arange(len(years))
    ax2 = ax.twinx()
    ax.bar(xa, nuevo, width=0.55, color=PAL[:len(years)], alpha=0.80, edgecolor="none",
           label="Nuevos docentes")
    ax2.plot(xa, cumul, color="#FFB74D", linewidth=2.5, marker="o", markersize=9,
             label="Acumulado")
    mv = max(nuevo) if nuevo else 1
    for i,(v,c_v) in enumerate(zip(nuevo,cumul)):
        ax.text(i, v+mv*0.03, str(v), ha="center", va="bottom", fontsize=11,
                fontweight="bold", color="white",
                path_effects=[pe.withStroke(linewidth=2,foreground="#0A0F18")])
        ax2.text(i, c_v+mv*0.04, str(c_v), ha="center", va="bottom", fontsize=9,
                 color="#FFB74D",
                 path_effects=[pe.withStroke(linewidth=1.5,foreground="#0A0F18")])
    ax.set_xticks(xa); ax.set_xticklabels([str(y) for y in years], fontsize=11, color="white")
    _style(ax, xlabel="Año de primera formación")
    ax.set_ylim(0, mv*1.35)
    ax2.tick_params(axis="y", colors="#FFB74D", labelsize=9)
    ax2.spines["right"].set_edgecolor("#FFB74D"); ax2.spines["right"].set_alpha(0.5)
    for sp in ["top","left","bottom"]: ax2.spines[sp].set_visible(False)
    lines1, labs1 = ax.get_legend_handles_labels()
    lines2, labs2 = ax2.get_legend_handles_labels()
    ax.legend(lines1+lines2, labs1+labs2, fontsize=9, framealpha=0.2,
              labelcolor="white", facecolor="#101820", edgecolor="#444")

    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"16_chart.png"), prs)
    _T(sl, "Evolución de Incorporación a Formación por Año (Docentes Aptos P3)")
    _POP(sl)
    _CT(sl, f"Eje izq: nuevos docentes por año  ·  Eje der: acumulado  ·  unidad=docente, n={len(first)}")
    anio_pk = years[nuevo.index(max(nuevo))] if nuevo else "N/D"
    _BUL(sl, [
        f"El año de mayor incorporación fue {anio_pk} con {max(nuevo)} docentes Aptos P3 "
        "que iniciaron su formación ese año.",
        f"Al cierre del período, {cumul[-1]} docentes acumulan al menos una formación válida "
        "para el análisis P3. Cada docente se cuenta solo una vez (primer año de formación).",
        "La curva acumulada muestra el crecimiento sostenido del programa de formación docente UCEN.",
    ])
    print("  ✓ slide 16 — Evolución acumulativa")

def slide_17(prs):
    """Jerarquía × Tipo — (era slide_18). 'Participación Mixta' en lugar de 'Combinado'."""
    ...
    df["tipo_s"] = df["tipos_formacion"].str.strip()
    def _tipo_simple(t):
        t = t.upper()
    ...
    df["tipo_g"] = df["tipo_s"].apply(_tipo_simple)
    pivot = (df.groupby(["jer_u","tipo_g"])["rut_key"].count()
             .unstack(fill_value=0))
    order = [j for j in JER_ORD_U if j in pivot.index]
    pivot = pivot.reindex(order)
    pivot = pivot.loc[pivot.sum(axis=1)>0]
    jer_lbl = [j.title() for j in pivot.index.tolist()]
    tipos   = pivot.columns.tolist()

    fig = _tr_fig()
    ax  = fig.add_axes([CHART_X, CHART_Y, CHART_W, CHART_H], facecolor="none", zorder=5)
    n   = len(jer_lbl); ya = np.arange(n); h = 0.55/max(len(tipos),1)
    for i,tipo in enumerate(tipos):
        vals   = pivot[tipo].tolist()
        offset = (i-(len(tipos)-1)/2)*h
        col = TIPO_COLS.get(tipo, PAL[i % len(PAL)])
        ax.barh(ya[::-1]+offset, vals, height=h*0.88, color=col,
                alpha=0.90, edgecolor="none", label=tipo)
        for j,v in enumerate(vals[::-1]):
            if v > 0:
                ax.text(v+0.2, j+offset, str(v), va="center", ha="left",
                        fontsize=8, fontweight="bold", color="white",
                        path_effects=[pe.withStroke(linewidth=1.5,foreground="#0A0F18")])
    ax.set_yticks(ya)
    ax.set_yticklabels(jer_lbl[::-1], fontsize=9, color="white")
    ...
    _POP(sl)
    _CT(sl, f"Formados Aptos P3: Jerarquía × Tipo  (n={len(df)} con dato, valores en barras)")
    jer_top = jer_lbl[0] if jer_lbl else "N/D"
    _BUL(sl, [
        f"'{jer_top}' concentra la mayor participación en formación entre los 197 Aptos P3.",
        "Las jerarquías de entrada (Instructor, Asistente) muestran mayor orientación a la formación "
        "que las superiores — patrón consistente con la antigüedad.",
        "Los Talleres son más transversales entre jerarquías; los Diplomados se concentran más "
        "en jerarquías intermedias (Asistente, Asociado).",
    ])
    print("  ✓ slide 17 — Jerarquía × Tipo")

def slide_18(prs):
    """Antigüedad × Tipo — (era slide_19). N DENTRO de cada barra. 'Participación Mixta'."""
    ...
    df["tipo_g"] = df["tipos_formacion"].apply(_tipo_s)
    ...
    fig = _tr_fig()
    ax  = fig.add_axes([CHART_X,CHART_Y,CHART_W,CHART_H], facecolor="none", zorder=5)
    n   = len(ant_lbl); xa = np.arange(n); w = 0.65/max(len(tipos),1)
    all_vals = [pivot.loc[t,tipo] for t in ant_lbl for tipo in tipos if t in pivot.index]
    ...
    for i,tipo in enumerate(tipos):
        ...
        col = TIPO_COLS.get(tipo, PAL[i % len(PAL)])
        ax.bar(xa+offset, vals, width=w*0.88, color=col,
               alpha=0.90, edgecolor="none", label=tipo)
        # n FUERA de la barra, encima, en vertical
        for j,v in enumerate(vals):
            if v > 0:
                ax.text(xa[j]+offset, v + mv*0.025, str(int(v)),
    ...
    _POP(sl)
    _CT(sl, f"Formados Aptos P3: Antigüedad × Tipo  (n={len(df)} con dato  ·  n dentro de cada barra)")
    _BUL(sl, [
        "Los docentes con menor antigüedad (0–9 años) participan con mayor frecuencia en formación, "
        "concentrando la mayoría de los Aptos P3.",
        "A mayor antigüedad, la participación decae, con variación según el tipo de modalidad.",
        "Los Talleres muestran participación más homogénea entre tramos; los Diplomados "
        "se concentran en antigüedades intermedias (5–14 años).",
    ])
    print("  ✓ slide 18 — Antigüedad × Tipo")

def slide_19(prs):
    """Intensidad — (era slide_20). Sin cambios de contenido."""
...
def slide_20(prs):
    """Combinaciones — (era slide_21). 'Participación Mixta' en texto."""
...
def slide_21(prs):
    """Perfil Control — (era slide_22). _fac() ya corregido globalmente."""
...
def slide_23(prs):
    """Trayectoria Pura — (era slide_25). Delta visible: flecha doble + sombreado."""
    def _pop_pura(df):
        sT = df[(df["n_taller"]>0)&(df["n_diplomado"]==0)&(df["n_proyecto"]==0)]
        sD = df[(df["n_diplomado"]>0)&(df["n_taller"]==0)&(df["n_proyecto"]==0)]
        sP = df[(df["n_proyecto"]>0)&(df["n_taller"]==0)&(df["n_diplomado"]==0)]
        return {"Taller":sT, "Diplomado":sD, "Proyecto":sP}

    puras = _pop_pura(sat)
    colores = {"Taller":"#5C9BD6","Diplomado":"#FFB74D","Proyecto":"#A5D6A7"}

    fig,ax = _ax()
    ax.set_xlim(-0.55, 1.90)

    for tipo,gdf in puras.items():
        if len(gdf) < 2: continue
        bl  = gdf["z_baseline"].mean()
        res = gdf["z_resultado"].mean()
        delta = res - bl
        col  = colores.get(tipo,"#FFFFFF")
        fill_col = "#A5D6A7" if delta >= 0 else "#F48FB1"

        # Sombreado entre la línea y el baseline
        ax.fill_between([0, 1], [bl, res], [bl, bl], alpha=0.18, color=fill_col, zorder=2)

        # Línea de trayectoria
        ax.plot([0, 1], [bl, res], color=col, linewidth=3.0, marker="o", markersize=12,
                label=f"{tipo}  (n={len(gdf)})", zorder=5)

        # Etiquetas en endpoints
        ax.text(-0.08, bl, f"{bl:.3f}", ha="right", va="center", fontsize=10,
                fontweight="bold", color=col,
                path_effects=[pe.withStroke(linewidth=2, foreground="#0A0F18")])
        ax.text(1.08, res, f"{res:.3f}", ha="left", va="center", fontsize=10,
                fontweight="bold", color=col,
                path_effects=[pe.withStroke(linewidth=2, foreground="#0A0F18")])

        # Flecha doble + etiqueta Δ en el margen derecho (x=1.45)
        mid_y = (bl + res) / 2
        ax.annotate("", xy=(1.45, res), xytext=(1.45, bl),
                    arrowprops=dict(arrowstyle="<->", color=col, lw=1.8))
        dz_str = f"Δ={delta:+.3f}"
        ax.text(1.52, mid_y, dz_str, ha="left", va="center", fontsize=9,
                fontweight="bold", color=fill_col,
                path_effects=[pe.withStroke(linewidth=2, foreground="#0A0F18")])

    ax.axhline(0, color="white", linewidth=1, linestyle=":", alpha=0.5)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Baseline (antes)", "Resultado (después)"],
    ...
    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"23_chart.png"), prs)
    _T(sl, "Trayectoria SAT: Baseline → Resultado (Poblaciones Puras)")
    _POP(sl, "Población pura: docentes con un solo tipo de formación  ·  excluye Participación Mixta")
    _CT(sl, "z-score SAT promedio — solo docentes con único tipo de formación  ·  Δ = resultado − baseline (flecha derecha)")
    nT = len(puras["Taller"]); nD = len(puras["Diplomado"]); nP = len(puras["Proyecto"])
    dT = (puras["Taller"]["z_resultado"].mean()-puras["Taller"]["z_baseline"].mean()) if nT>1 else 0
    dD = (puras["Diplomado"]["z_resultado"].mean()-puras["Diplomado"]["z_baseline"].mean()) if nD>1 else 0
    _BUL(sl, [
        f"Población pura Taller (n={nT}): Δz = {dT:+.3f}  ·  "
        f"Población pura Diplomado (n={nD}): Δz = {dD:+.3f}. "
        f"Las flechas derechas muestran la magnitud del cambio por modalidad.",
        "Al aislar docentes con un solo tipo de formación se elimina el efecto "
        "de la Participación Mixta, obteniendo el impacto neto de cada modalidad.",
        "El sombreado muestra la dirección del cambio: verde=mejora, rosa=descenso.",
    ])
    print("  ✓ slide 23 — Trayectoria Pura")

def _tipo_simple(t):
    """Helper reutilizable: clasifica tipos_formacion en 4 categorías."""
    t = str(t).upper()
    if "TALLER" in t and "DIPLOMADO" not in t and "PROYECTO" not in t: return "Taller"
    if "DIPLOMADO" in t and "TALLER" not in t and "PROYECTO" not in t: return "Diplomado"
    if "PROYECTO" in t and "TALLER" not in t and "DIPLOMADO" not in t: return "Proyecto"
...
def slide_24(prs):
    """SAT por Facultad — z_baseline vs z_resultado por unidad."""
    ...
    fig = _tr_fig()
    ax  = fig.add_axes([CHART_X, CHART_Y, CHART_W, CHART_H], facecolor="none", zorder=5)
    n   = len(facs); ya = np.arange(n); h = 0.35
    ax.barh(ya - h/2, z_b, height=h, color="#5C9BD6", alpha=0.85, edgecolor="none", label="Baseline")
    ax.barh(ya + h/2, z_r, height=h, color="#A5D6A7", alpha=0.85, edgecolor="none", label="Resultado")
    for i,(b,r,nv) in enumerate(zip(z_b, z_r, n_fac)):
        col_arr = "#4CAF50" if r > b else "#EF5350"
        ax.annotate("", xy=(r, i+h/2), xytext=(b, i-h/2),
                    arrowprops=dict(arrowstyle="->", color=col_arr, lw=1.5, connectionstyle="arc3,rad=0"))
        xmax = max(abs(b), abs(r)) * 1.05
        ax.text(max(b,r) + abs(xmax)*0.08 + 0.05, i, f"n={nv}",
                va="center", ha="left", fontsize=7.5, color="#AAAAAA")
    ...
    _style(ax); ax.set_ylabel("z-score SAT promedio", color="#AAAAAA", fontsize=9)
    xpad = max([abs(v) for v in z_b+z_r]+[0.1]) * 0.55
    ax.set_xlim(min(z_b+z_r)-0.15, max(z_b+z_r)+xpad)
    ...
    _POP(sl)
    _CT(sl, "z-score SAT promedio por unidad/facultad  ·  azul=baseline  ·  verde=resultado  ·  flechas=dirección del cambio")
    z_b_m = float(np.mean(z_b)); z_r_m = float(np.mean(z_r))
    _BUL(sl, [
        f"z promedio de los formados: Baseline={z_b_m:.3f} → Resultado={z_r_m:.3f} "
        f"(Δ global={z_r_m-z_b_m:+.3f}). La mayoría de las facultades muestra mejora.",
        "Las flechas indican la dirección del cambio por facultad: verde=mejora, rojo=descenso.",
        "z=0 es el promedio de docentes de la misma facultad ese semestre — z positivo indica "
        "que los formados superan el promedio de su facultad.",
    ])
    print("  ✓ slide 24 — SAT por Facultad")


def slide_25(prs):
    """Histograma de Δz por tipo de formación."""
    df = sat[sat["delta_z"].notna()].copy()
    df["tipo_g"] = df["tipos_formacion"].apply(_tipo_simple)

    fig, ax = _ax()
    bins = np.linspace(df["delta_z"].min()-0.1, df["delta_z"].max()+0.1, 22)
    for tipo, col in TIPO_COLS.items():
        sub = df[df["tipo_g"]==tipo]
        if len(sub) == 0: continue
        ax.hist(sub["delta_z"], bins=bins, color=col, alpha=0.65,
                label=f"{tipo} (n={len(sub)})", edgecolor="none")
    ax.axvline(0, color="white", linewidth=1.5, linestyle="--", alpha=0.7)
    med_all = df["delta_z"].median()
    ymax = ax.get_ylim()[1]
    ax.axvline(med_all, color="#FFB74D", linewidth=1.5, linestyle="-", alpha=0.85)
    ax.text(med_all + (df["delta_z"].max()-df["delta_z"].min())*0.03,
            ymax * 0.88, f"Mediana={med_all:+.2f}",
            color="#FFB74D", fontsize=9, fontweight="bold",
            path_effects=[pe.withStroke(linewidth=1.5, foreground="#0A0F18")])
    _style(ax, xlabel="Δ z-score (resultado − baseline)")
    ax.set_ylabel("Frecuencia", color="#AAAAAA", fontsize=9)
    ax.legend(fontsize=9, framealpha=0.2, labelcolor="white", facecolor="#101820", edgecolor="#444")

    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"25_chart.png"), prs)
    _T(sl, "Distribución del Cambio SAT — Histograma por Tipo (Δ z-score)")
    _POP(sl)
    _CT(sl, "Distribución de Δz = z_resultado − z_baseline  ·  línea naranja = mediana general  ·  n=197")
    n_pos = len(df[df["delta_z"]>0]); n_neg = len(df[df["delta_z"]<0])
    _BUL(sl, [
        f"De los {N197} formados, {n_pos} ({100*n_pos/N197:.0f}%) mejoraron su z-score y "
        f"{n_neg} ({100*n_neg/N197:.0f}%) bajaron. Mediana general: {med_all:+.3f}.",
        "La distribución tiende hacia la mejora, con mayor peso en los valores positivos.",
        "El Diplomado concentra sus cambios en valores positivos, consistente con "
        "mayor efecto de la formación de larga duración.",
    ])
    print("  ✓ slide 25 — Histograma Δz")


def slide_26(prs):
    """Cambio SAT × Antigüedad × Tipo — barras de media delta_z."""
    ...
    df = sat[sat["delta_z"].notna() & sat["antiguedad_anios"].notna()].copy()
    df["tramo"]  = df["antiguedad_anios"].apply(_tramo)
    df["tipo_g"] = df["tipos_formacion"].apply(_tipo_simple)
    df = df[df["tramo"].notna()]
    pivot = df.groupby(["tramo","tipo_g"])["delta_z"].mean().unstack(fill_value=0)
    pivot = pivot.reindex([t for t in ORD_ANT if t in pivot.index])
    tipos = pivot.columns.tolist(); ant_lbl = pivot.index.tolist()

    fig, ax = _ax()
    xa = np.arange(len(ant_lbl)); w = 0.65/max(len(tipos),1)
    for i, tipo in enumerate(tipos):
        vals   = [float(pivot.loc[t,tipo]) if t in pivot.index else 0 for t in ant_lbl]
        offset = (i-(len(tipos)-1)/2)*w
        col = TIPO_COLS.get(tipo, PAL[i % len(PAL)])
        ax.bar(xa+offset, vals, width=w*0.88, color=col, alpha=0.90, edgecolor="none", label=tipo)
        for j, v in enumerate(vals):
            if abs(v) > 0.01:
                ax.text(xa[j]+offset, v + (0.005 if v >= 0 else -0.005),
    ...
    ax.set_xticks(xa); ax.set_xticklabels(ant_lbl, fontsize=11, color="white")
    _style(ax, xlabel="Tramo de antigüedad (años)")
    ax.set_ylabel("Δ z-score promedio", color="#AAAAAA", fontsize=9)
    ax.legend(fontsize=9, framealpha=0.2, labelcolor="white", facecolor="#101820", edgecolor="#444")

    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"26_chart.png"), prs)
    _T(sl, "Cambio SAT (Δz) por Antigüedad y Tipo de Formación")
    _POP(sl)
    _CT(sl, "Media de Δz por tramo de antigüedad × tipo  ·  Δz = z_resultado − z_baseline  ·  n varía por grupo")
    dz_global = df["delta_z"].mean()
    _BUL(sl, [
        f"La mejora SAT (Δz promedio global={dz_global:+.3f}) varía según antigüedad y tipo. "
        "El Diplomado tiende a mayor cambio positivo.",
        "Los docentes con menor antigüedad (0–9 años) concentran mayor volumen de participantes "
        "y muestran mejora consistente en Taller.",
        "El efecto diferencial por antigüedad sugiere que el impacto es mayor en docentes "
        "más nuevos en la institución, independiente del tipo de formación.",
    ])
    print("  ✓ slide 26 — Cambio SAT × Antigüedad × Tipo")


def slide_27(prs):
    """SAT por Jerarquía — Taller puro: baseline → resultado."""
    JER_ORD = ["INSTRUCTOR REGULAR","INSTRUCTOR DOCENTE","ASISTENTE REGULAR","ASISTENTE DOCENTE",
               "ASOCIADO REGULAR","ASOCIADO DOCENTE","TITULAR REGULAR","TITULAR DOCENTE"]
    ...
    ax.bar(xa - w/2, z_b, width=w, color="#5C9BD6", alpha=0.85, edgecolor="none", label="Baseline")
    ax.bar(xa + w/2, z_r, width=w, color="#4CAF50", alpha=0.85, edgecolor="none", label="Resultado")
    for i,(b,r,nv) in enumerate(zip(z_b, z_r, n_j)):
        col_arr = "#4CAF50" if r > b else "#EF5350"
        ax.annotate("", xy=(xa[i]+w/2, r), xytext=(xa[i]-w/2, b),
                    arrowprops=dict(arrowstyle="->", color=col_arr, lw=1.2,
                                   connectionstyle="arc3,rad=-0.2"))
        ax.text(xa[i], min(b,r) - 0.04, f"n={nv}", ha="center", va="top",
                fontsize=7.5, color="#AAAAAA")
    ax.axhline(0, color="white", linewidth=1, linestyle=":", alpha=0.5)
    ax.set_xticks(xa); ax.set_xticklabels(jers, fontsize=8.5, color="white", rotation=20, ha="right")
    _style(ax); ax.set_ylabel("z-score SAT promedio", color="#AAAAAA", fontsize=9)
    ax.legend(fontsize=9, framealpha=0.2, labelcolor="white", facecolor="#101820", edgecolor="#444")

    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"27_chart.png"), prs)
    _T(sl, "SAT por Jerarquía — Población Pura Taller (Baseline → Resultado)")
    _POP(sl, "Población pura Taller: solo docentes con Taller  ·  excluye Diplomado y Proyecto")
    _CT(sl, "z-score SAT por jerarquía  ·  Taller puro  ·  azul=baseline  ·  verde=resultado  ·  n por jerarquía")
    n_tot = len(df)
    _BUL(sl, [
        f"Población pura Taller: {n_tot} docentes. La mayoría de las jerarquías muestra mejora "
        "SAT tras la formación en Taller.",
        "Las jerarquías de entrada (Instructor, Asistente) concentran mayor número de participantes "
        "en Taller — consistente con el perfil de los Aptos P3.",
        "Las flechas curvas conectan baseline y resultado por jerarquía: verde=mejora, rojo=descenso.",
    ])
    print("  ✓ slide 27 — SAT × Jerarquía Taller")


def slide_28(prs):
    """SAT por Jerarquía — Diplomado puro: baseline → resultado."""
    JER_ORD = ["INSTRUCTOR REGULAR","INSTRUCTOR DOCENTE","ASISTENTE REGULAR","ASISTENTE DOCENTE",
               "ASOCIADO REGULAR","ASOCIADO DOCENTE","TITULAR REGULAR","TITULAR DOCENTE"]
    df = sat[(sat["n_diplomado"]>0) & (sat["n_taller"]==0) & (sat["n_proyecto"]==0)].copy()
    if len(df) < 2:
        _pic(sl := _new_sl(prs), SHARED_BG, prs)
        _T(sl, "SAT por Jerarquía — Población Pura Diplomado")
        _txt(sl, f"Muestra insuficiente para Diplomado puro (n={len(df)}).",
             PIC_L+200000, PIC_T+PIC_H//2-100000, PIC_W-400000, 250000,
             fs=14, color="#90ABC4", align=PP_ALIGN.CENTER)
        print("  ✓ slide 28 — SAT × Jerarquía Diplomado (sin datos)")
        return
    df["jer_u"] = df["jerarquia"].str.strip().str.upper()
    grp = df.groupby("jer_u")[["z_baseline","z_resultado"]].mean()
    ord_pres = [j for j in JER_ORD if j in grp.index]
    ...
    if len(grp) == 0:
        _pic(sl := _new_sl(prs), SHARED_BG, prs)
        _T(sl, "SAT por Jerarquía — Población Pura Diplomado")
        _txt(sl, "Sin jerarquías con suficiente datos en Diplomado puro.",
             PIC_L+200000, PIC_T+PIC_H//2-100000, PIC_W-400000, 250000,
             fs=14, color="#90ABC4", align=PP_ALIGN.CENTER)
        print("  ✓ slide 28 — SAT × Jerarquía Diplomado (sin jerarquías)")
        return
    jers = [j.title() for j in grp.index.tolist()]
    z_b  = grp["z_baseline"].tolist(); z_r = grp["z_resultado"].tolist()
    n_j  = [int(df[df["jer_u"]==j]["rut_key"].count()) for j in grp.index.tolist()]
    ...
    fig, ax = _ax()
    xa = np.arange(len(jers)); w = 0.35
    ax.bar(xa - w/2, z_b, width=w, color="#FFB74D", alpha=0.85, edgecolor="none", label="Baseline")
    ax.bar(xa + w/2, z_r, width=w, color="#4CAF50", alpha=0.85, edgecolor="none", label="Resultado")
    for i,(b,r,nv) in enumerate(zip(z_b, z_r, n_j)):
        col_arr = "#4CAF50" if r > b else "#EF5350"
        ax.annotate("", xy=(xa[i]+w/2, r), xytext=(xa[i]-w/2, b),
    ...
    _pic(sl := _new_sl(prs), SHARED_BG, prs); _pic(sl, _save_ch(fig,"28_chart.png"), prs)
    _T(sl, "SAT por Jerarquía — Población Pura Diplomado (Baseline → Resultado)")
    _POP(sl, "Población pura Diplomado: solo docentes con Diplomado  ·  excluye Taller y Proyecto")
    _CT(sl, "z-score SAT por jerarquía  ·  Diplomado puro  ·  naranja=baseline  ·  verde=resultado")
    n_tot = len(df)
    _BUL(sl, [
        f"Población pura Diplomado: {n_tot} docentes (muestra pequeña — interpretar con cautela). "
        "La mayoría de las jerarquías presentes muestra mejora SAT tras el Diplomado.",
        "El Diplomado de larga duración tiende a producir un efecto más pronunciado que el Taller, "
        "visible en el Δz promedio de esta subpoblación.",
        "Las flechas conectan baseline y resultado: verde=mejora, rojo=descenso por jerarquía.",
    ])
    print("  ✓ slide 28 — SAT × Jerarquía Diplomado")
...
def slide_scatter_panels(prs):
    ...
    _BUL(sl, [
        ...
        "Los docentes formados (naranja) se concentran en la región SAT mayor a 5 con "
        "notas promedio superiores; los sin formación presentan mayor dispersión hacia "
...
if __name__ == "__main__":
    print("Generando PRESENTACION_197_P3_final.pptx (33 slides) …")
    _ensure_bg()
    prs = Presentation()
    ...
    slide_embudo_v2(prs)           # 14 Embudo 917→357→197 (compacto)
    slide_15(prs)                  # 15 Sep 2.1 Caracterización
    slide_17(prs)                  # 16 Participación × Jerarquía
    slide_18(prs)                  # 17 Participación × Antigüedad
    slide_19(prs)                  # 18 Intensidad participación
    slide_20(prs)                  # 19 Combinaciones modalidad
    slide_21(prs)                  # 20 Perfil grupo control
    slide_22(prs)                  # 21 SAT z formados vs control (6 períodos)
    slide_24(prs)                  # 22 SAT z por facultad

    # BLOQUE III — Aprobación de Alumnos
    slide_29(prs)                  # 23 Sep BLOQUE III
    slide_scatter_panels(prs)      # 24 Scatter SAT vs Nota (3 paneles)
    slide_30(prs)                  # 25 Aprobación global formados vs control
    slide_31(prs)                  # 26 Evolución aprobación × período
    slide_32(prs)                  # 27 Aprobación × Antigüedad
    slide_33(prs)                  # 28 Aprobación × Jerarquía
    slide_34(prs)                  # 29 Efecto acumulativo × instancias

    # BLOQUE IV — EDD
    slide_36_bloque4_sep(prs)      # 30 Sep BLOQUE IV
    slide_37(prs)                  # 31 Evolución EDD formados vs control
    slide_38(prs)                  # 32 EDD por tipo de formación
    slide_35(prs)                  # 33 Conclusiones y Recomendaciones

    OUT_PPTX_FINAL = OUT_PPTX.replace("v3_slides1_26", "final")
    prs.save(OUT_PPTX_FINAL)
    print(f"\n✓ Guardado: {OUT_PPTX_FINAL}")
